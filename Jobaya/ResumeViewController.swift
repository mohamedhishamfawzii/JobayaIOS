...
//  Jobaya
//
//  Created by mohamed hisham on 6/26/19.
...
class ResumeViewController: UIViewController {
    ...
    @IBAction func sumbit(_ sender: Any) {
        var request = URLRequest(url: URL(string: "https://jobayaback.herokuapp.com/resume")!)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.timeoutInterval = 120 // 120 secs
        let education = mySearchTextField.text
        let exp = expTextField.text
        let skills = skillsTextField.text
        let values =  ["education":education,"email":UserDefaults.standard.string(forKey: "email"),"experience":exp,"skills":skills ,"gender":gender]
        print (values)
        request.httpBody = try! JSONSerialization.data(withJSONObject: values, options: [])
        Alamofire.request(request as URLRequestConvertible).responseString {
            response in
            let alert = UIAlertController(title: "Success", message: "You have added your resume", preferredStyle: .alert)
